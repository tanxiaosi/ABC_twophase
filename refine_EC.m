function  New_Mesh = refine_EC(Old_Mesh,varargin)
...
%   CH-8092 Zurich, Switzerland
  ...
  % Red refinement for quadrilateral elements

    % Preallocate memory
        
    New_Mesh.Coordinates = zeros(nCoordinates+nEdges+nElements,2);
    New_Mesh.Elements = zeros(4*nElements,4); 
    New_Mesh.HBd = zeros(4*nElements,2); 
    New_Mesh.VBd = zeros(4*nElements,2); 
    ...
    for i = 1:nElements
      ...
      if(BdFlag_1 < 0)
        New_Mesh.HBd(4*(i-1)+[1;2],:) = [-1,H_para;-1,H_para]; 
        H_para = H_para + 1;
        if(~isempty(DHandle))
        ...
        H_aux = find(New_Mesh.HBd(:,1)==Old_Mesh.Vert2Edge(i1,i2));
        if size(H_aux,1) == 0
            New_Mesh.HBd(4*(i-1)+[1;2],:)=[Old_Mesh.Vert2Edge(i1,i2),H_para_In;Old_Mesh.Vert2Edge(i1,i2),H_para_In];
            H_para_In = H_para_In + 1;
        else
            New_Mesh.HBd(4*(i-1)+[1;2],:)=[Old_Mesh.Vert2Edge(i1,i2),New_Mesh.HBd(H_aux(1),2);Old_Mesh.Vert2Edge(i1,i2),New_Mesh.HBd(H_aux(1),2)];
        end
        New_Mesh.Coordinates(j1,:) = 1/2*(Old_Mesh.Coordinates(i1,:)+Old_Mesh.Coordinates(i2,:)); 
      end
      ...
      if(BdFlag_2 < 0)
          New_Mesh.VBd(4*(i-1)+[2;4],:) = [-1,V_para;-1,V_para]; 
          V_para = V_para + 1;
        if(~isempty(DHandle))
        ...
        V_aux = find(New_Mesh.VBd(:,1)==Old_Mesh.Vert2Edge(i2,i3));
        if size(V_aux,1) == 0
            New_Mesh.VBd(4*(i-1)+[2;4],:)=[Old_Mesh.Vert2Edge(i2,i3),V_para_In;Old_Mesh.Vert2Edge(i2,i3),V_para_In];
            V_para_In = V_para_In + 1;
        else
            New_Mesh.VBd(4*(i-1)+[2;4],:)=[Old_Mesh.Vert2Edge(i2,i3),New_Mesh.VBd(V_aux(1),2);Old_Mesh.Vert2Edge(i2,i3),New_Mesh.VBd(V_aux(1),2)];
        end
        New_Mesh.Coordinates(j2,:) = 1/2*(Old_Mesh.Coordinates(i2,:)+Old_Mesh.Coordinates(i3,:));
      end
      ...
      if(BdFlag_3 < 0)
          New_Mesh.HBd(4*(i-1)+[3;4],:) = [-1,H_para;-1,H_para]; 
          H_para = H_para + 1;
        if(~isempty(DHandle))
        ...
        H_aux = find(New_Mesh.HBd(:,1)==Old_Mesh.Vert2Edge(i3,i4));
        if size(H_aux,1) == 0
            New_Mesh.HBd(4*(i-1)+[3;4],:)=[Old_Mesh.Vert2Edge(i3,i4),H_para_In;Old_Mesh.Vert2Edge(i3,i4),H_para_In];
            H_para_In = H_para_In + 1;
        else
            New_Mesh.HBd(4*(i-1)+[3;4],:)=[Old_Mesh.Vert2Edge(i3,i4),New_Mesh.HBd(H_aux(1),2);Old_Mesh.Vert2Edge(i3,i4),New_Mesh.HBd(H_aux(1),2)];
        end
        New_Mesh.Coordinates(j3,:) = 1/2*(Old_Mesh.Coordinates(i3,:)+Old_Mesh.Coordinates(i4,:));
      end
      ...
      if(BdFlag_4 < 0)
          New_Mesh.VBd(4*(i-1)+[1;3],:) = [-1,V_para;-1,V_para]; 
          V_para = V_para + 1; 
        if(~isempty(DHandle))
        ...
        V_aux = find(New_Mesh.VBd(:,1)==Old_Mesh.Vert2Edge(i4,i1));
        if size(V_aux,1) == 0
            New_Mesh.VBd(4*(i-1)+[1;3],:)=[Old_Mesh.Vert2Edge(i4,i1),V_para_In;Old_Mesh.Vert2Edge(i4,i1),V_para_In];
            V_para_In = V_para_In + 1;
        else
            New_Mesh.VBd(4*(i-1)+[1;3],:)=[Old_Mesh.Vert2Edge(i4,i1),New_Mesh.VBd(V_aux(1),2);Old_Mesh.Vert2Edge(i4,i1),New_Mesh.VBd(V_aux(1),2)];
        end
        New_Mesh.Coordinates(j4,:) = 1/2*(Old_Mesh.Coordinates(i4,:)+Old_Mesh.Coordinates(i1,:));
      end
